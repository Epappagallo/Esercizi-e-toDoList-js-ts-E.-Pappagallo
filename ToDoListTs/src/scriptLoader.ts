...
function init(): void {
  ...
  const currentScriptSpan = document.getElementById("current-script-name") as HTMLElement;
  ...
  if (controlsDiv) {
    controlsDiv.addEventListener("click", (event: MouseEvent) => {
      const buttonId = (event.target as HTMLElement).id;
      const scriptName = scripts[buttonId as keyof typeof scripts];
